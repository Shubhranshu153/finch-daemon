...
func Create(ctx context.Context, client *containerd.Client, args []string, netManager containerutil.NetworkOptionsManager, options types.ContainerCreateOptions) (containerd.Container, func(), error) {
	...
	// TODO: Move up the stack when the userns functionality is available in nerdctl
	userns := ""
	for _, label := range options.Label {
		if strings.HasPrefix(label, "runfinch.com/internal/userns=") {
			userns = strings.TrimPrefix(label, "runfinch.com/internal/userns=")
		}
	}
	...
	if userns != "" {
		...
		caps, err := client.GetSnapshotterCapabilities(ctx, ensuredImage.Snapshotter)
		supportsMultiRemapIds := false
		for _, cap := range caps {
			if cap == capabMultiRemapIDs {
				supportsMultiRemapIds = true
				break
			}
		}
		if uidmaps, gidmaps := idMapping.UIDMaps, idMapping.GIDMaps; len(uidmaps) > 0 && len(gidmaps) > 0 {
			uidmappings := convertIDMapToLinuxIDMapping(uidmaps)
			gidmappings := convertIDMapToLinuxIDMapping(gidmaps)
			if err != nil {
				return nil, nil, err
			}
			opts = append(opts, oci.WithUserNamespace(uidmappings, gidmappings))

			// containerd overlayfs doesnt support remapids in 1.7.x
			if len(uidmaps) > 1 && len(gidmaps) > 1 && supportsMultiRemapIds {
				cOpts = append(cOpts, containerd.WithNewSnapshot(id, ensuredImage.Image, WithUserNSRemapperLabels(uidmappings, gidmappings)))

			} else if len(uidmaps) > 1 && len(gidmaps) > 1 {
				return nil, nil, errors.New("snapshotter doesn't support remapping multiple uids/gids is not supported (FIXME)")
			} else {
				fmt.Printf("in single remap ids logic")
				cOpts = append(cOpts, containerd.WithNewSnapshot(id, ensuredImage.Image, containerd.WithRemapperLabels(0, uidmappings[0].HostID, 0, gidmappings[0].HostID, uidmappings[0].Size)))
			}
		}
	} else {
