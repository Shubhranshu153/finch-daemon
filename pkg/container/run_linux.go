...
import (
	...
	idmap "github.com/runfinch/finch-daemon/pkg/userns"
)
...
func convertIDMapSliceToStringSlice(idMaps []idtools.IDMap) []string {
	var result []string
	for _, idMap := range idMaps {
		// Construct the string "containerID:HostID:Size"
		result = append(result, strconv.Itoa(idMap.ContainerID)+":"+
			strconv.Itoa(idMap.HostID)+":"+
			strconv.Itoa(idMap.Size))
	}
	return result
}
...
func loadIdentityMapping(userns string) (idtools.IdentityMapping, error) {
	return idtools.LoadIdentityMapping(userns)
}
...
func WithUserNSRemapperLabels(uidmaps, gidmaps []specs.LinuxIDMapping) snapshots.Opt {
	...
	fmt.Println(uidmapLabel, gidmapLabel)
